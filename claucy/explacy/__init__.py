...
def print_parse_info(sent=None,nlp=None):
    """ Print the dependency tree of `sent` (sentence)."""

    doc = None
    if hasattr(sent,"as_doc"):
        doc = sent.as_doc()
    else:
        assert nlp != None
    ...
    rows = [['Dep tree', 'Token', 'Lemma', 'Dep type', 'Part of Sp','FG PoS']]
    for i, token in enumerate(doc):
        rows.append([lines[i], token, token.lemma_, token.dep_, token.pos_, token.tag_])
    return _print_table(rows)

def explacy(*args,**kwargs):
    print(print_parse_info(*args,**kwargs))
